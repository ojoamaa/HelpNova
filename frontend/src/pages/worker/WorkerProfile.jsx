...
export default function WorkerProfile() {
    ...
    async function saveProfile(event) {
        ...
        try {
            ...
            if ([404, 405, 501].includes(error?.response?.status) || !error?.response) {
                ...
                setNotice({
                    type: "error",
                    text: readableApiError(error, "Profile update failed."),
                });
            }
        } finally {
            setSaving(false);
    ...
    async function handlePhoto(event) {
        ...
        try {
            ...
            if ([404, 405, 501].includes(error?.response?.status) || !error?.response) {
                ...
                setNotice({ type: "error", text: readableApiError(error, "Photo upload failed.") });
            }
        } finally {
            setUploading(false);
        }
    ...
    async function toggleAvailability() {
        ...
        try {
            ...
            setNotice({ type: "error", text: readableApiError(error, "Availability update failed.") });
        }
    }
...
function Notice({ notice, onClose }) {
    ...
    if (typeof notice?.text === "string") {
        message = notice.text;
    } else if (Array.isArray(notice?.text)) {
        message = notice.text
            .map((item) =>
                typeof item === "string"
                    ? item
            ...
            .filter(Boolean)
            .join("; ");
    } else if (notice?.text && typeof notice.text === "object") {
        message =
            notice.text.msg ||
            notice.text.message ||
            JSON.stringify(notice.text);
    }

    return (
        <div
            className={`rounded-xl border p-3 flex items-start justify-between gap-3 ${styles}`}
        >
